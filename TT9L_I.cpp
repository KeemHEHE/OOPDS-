...
class MyVector {
    private:
        T* arr;    //pointer to a dynamic array that stores elements.
        int cap;   // current capacity (how many slots the array can hold).
        int count;  //current size (how many elements are actually stored).

        void grow() {     //purpose to doubles the capacity when the array is full.
            cap = cap * 2;
            T* newArr = new T[cap];
    ...
    public:
        ...
        // Cleans up memory when the object is destroyed. Prevents memory leaks.
        ~MyVector() {
            delete[] arr;
        }
        ...
        void push_back(T val) {
            if (count == cap) grow();    //If full (count == cap), call grow() to expand.
            arr[count] = val;            //Store the value at arr[count].
        ...
        bool isEmpty() { 
            return count == 0;        // Otherwise false.
        }
};
...
// Member 2: Adeeb
class MovInstruction : public Instruction {     //move a plain number into register like MOV(R1, 5)
    private:
        int dest;
